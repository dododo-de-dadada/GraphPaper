...
namespace winrt::GraphPaper::implementation
{
	...
	struct Shape {
		...
		// 部位の位置を得る.
		virtual	void get_anch_pos(const ANCH_WHICH /*anch*/, D2D1_POINT_2F&/*value*/) const noexcept {}
		// 開始位置を得る.
		...
		virtual void set_anch_pos(const D2D1_POINT_2F /*value*/, const ANCH_WHICH /*anch*/) {}
		// 値を選択フラグに格納する.
	...
	struct ShapeSheet : Shape {
		...
		// 値を用紙の色に格納する.
		void set_sheet_color(const D2D1_COLOR_F& value) noexcept;
		// 値を用紙の寸法に格納する.
	...
	struct ShapeStroke : Shape {
		...
		D2D1_POINT_2F m_diff{ 0.0f, 0.0f };	// 開始位置と終了位置との差分
		...
		// 図形を作成する.
		ShapeStroke(const ShapeSheet* attr);
		// 図形をデータリーダーから読み込む.
	...
	struct ShapeLine : ShapeStroke {
		...
		// 図形を作成する.
		ShapeLine(const D2D1_POINT_2F s_pos, const D2D1_POINT_2F diff, const ShapeSheet* attr);
		// 図形をデータリーダーから読み込む.
		...
		void set_arrow_style(const ARROW_STYLE value);
		// 部位の位置を得る.
		void get_anch_pos(const ANCH_WHICH anch, D2D1_POINT_2F& value) const noexcept;
		//	値を, 部位の位置に格納する. 他の部位の位置は動かない. 
		void set_anch_pos(const D2D1_POINT_2F value, const ANCH_WHICH anch);
		// 値を始点に格納する. 他の部位の位置も動く.
		void set_start_pos(const D2D1_POINT_2F value);
		// 差分だけ移動する.
		void move(const D2D1_POINT_2F diff);
	...
	struct ShapeRect : ShapeStroke {
		...
		// 図形を作成する.
		ShapeRect(const D2D1_POINT_2F s_pos, const D2D1_POINT_2F diff, const ShapeSheet* s_attr);
		// 図形をデータリーダーから読み込む.
	...
	struct ShapeElli : ShapeRect {
		// 図形を作成する.
		ShapeElli(const D2D1_POINT_2F s_pos, const D2D1_POINT_2F diff, const ShapeSheet* attr) :
	...
	struct ShapeRRect : ShapeRect {
		...
		// 図形を作成する.
		ShapeRRect(const D2D1_POINT_2F s_pos, const D2D1_POINT_2F diff, const ShapeSheet* attr);
		// 図形をデータリーダーから読み込む.
	...
	struct ShapePoly : ShapeStroke {
		...
		// 図形を作成する.
		ShapePoly(const ShapeSheet* attr);
		// 図形をデータリーダーから読み込む.
	...
	struct ShapeBezi : ShapePoly {
		...
		void set_start_pos(const D2D1_POINT_2F value);
		// 図形を作成する.
		ShapeBezi(const D2D1_POINT_2F s_pos, const D2D1_POINT_2F diff, const ShapeSheet* attr);
		// 図形をデータリーダーから読み込む.
	...
	struct ShapeText : ShapeRect {
		...
		bool adjust_bound(const D2D1_SIZE_F& bound = D2D1_SIZE_F{ 0.0F, 0.0F });
		// テキストレイアウトから計量の配列を得る.
		...
		// 図形を作成する.
		ShapeText(const D2D1_POINT_2F s_pos, const D2D1_POINT_2F diff, wchar_t* const text, const ShapeSheet* attr);
		// 図形をデータリーダーから読み込む.
