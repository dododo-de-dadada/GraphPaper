...
namespace winrt::GraphPaper::implementation
{
	...
	void MainPage::kacc_down_invoked(IInspectable const&, KeyboardAcceleratorInvokedEventArgs const&)
	{
		...
		const auto end = min(m_main_page.m_select_trail ? m_main_page.m_select_end + 1 : m_main_page.m_select_end, len);
		const auto start = min(m_main_page.m_select_start, len);
		const auto row = m_edit_text_shape->get_text_row(m_main_page.m_select_end);	// キャレットがある行
		const auto last = m_edit_text_shape->m_dwrite_test_cnt - 1;	// 最終行
		if (end != start && row == last) {
			...
			//const auto key_state = CoreWindow::GetForCurrentThread().GetKeyState(VirtualKey::Shift);
			//const auto shift_key = ((key_state & CoreVirtualKeyStates::Down) == CoreVirtualKeyStates::Down);
			const auto line_h = m_edit_text_shape->m_dwrite_test_metrics[row + 1].top - m_edit_text_shape->m_dwrite_test_metrics[row].top;
			D2D1_POINT_2F car;
			m_edit_text_shape->get_text_caret(end, row, m_main_page.m_select_trail, car);
	...
	void MainPage::kacc_up_invoked(IInspectable const&, KeyboardAcceleratorInvokedEventArgs const&)
	{
		...
		const auto end = min(m_main_page.m_select_trail ? m_main_page.m_select_end + 1 : m_main_page.m_select_end, len);
		const auto start = min(m_main_page.m_select_start, len);
		const auto row = m_edit_text_shape->get_text_row(m_main_page.m_select_end);
		if (end != start && row == 0) {
			//const auto key_state = CoreWindow::GetForCurrentThread().GetKeyState(VirtualKey::Shift);
			//const auto shift_key = ((key_state & CoreVirtualKeyStates::Down) == CoreVirtualKeyStates::Down);
			//if (shift_key) {
			...
			//const auto key_state = CoreWindow::GetForCurrentThread().GetKeyState(VirtualKey::Shift);
			//const auto shift_key = ((key_state & CoreVirtualKeyStates::Down) == CoreVirtualKeyStates::Down);
			const auto line_h = m_edit_text_shape->m_dwrite_test_metrics[row].top - m_edit_text_shape->m_dwrite_test_metrics[row - 1].top;
			D2D1_POINT_2F car;
			m_edit_text_shape->get_text_caret(end, row, m_main_page.m_select_trail, car);
			const D2D1_POINT_2F new_car{ car.x, car.y - line_h };
			bool new_trail;
