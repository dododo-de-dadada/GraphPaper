...
namespace winrt::GraphPaper::implementation
{
	...
	IAsyncAction MainPage::fill_color_click_async(IInspectable const&, RoutedEventArgs const&)
	{
		...
		D2D1_COLOR_F val;
		m_sample_sheet.get_fill_color(val);
		...
		const auto slider_0_token = sample_slider_0().ValueChanged({ this, &MainPage::fill_slider_val_changed<UNDO_OP::FILL_COLOR, 0> });
		const auto slider_1_token = sample_slider_1().ValueChanged({ this, &MainPage::fill_slider_val_changed<UNDO_OP::FILL_COLOR, 1> });
		const auto slider_2_token = sample_slider_2().ValueChanged({ this, &MainPage::fill_slider_val_changed<UNDO_OP::FILL_COLOR, 2> });
		const auto slider_3_token = sample_slider_3().ValueChanged({ this, &MainPage::fill_slider_val_changed<UNDO_OP::FILL_COLOR, 3> });
		...
		if (d_result == ContentDialogResult::Primary) {
			...
			m_sample_sheet.m_shape_list.back()->get_fill_color(samp_val);
