	...
	void MainPage::sample_panel_size_changed(IInspectable const&, RoutedEventArgs const&)
	{
		...
		if (m_sample_type != SAMPLE_TYPE::NONE) {
			if (m_sample_type == SAMPLE_TYPE::FONT) {
				...
				m_sample_shape->set_anch_pos(D2D1_POINT_2F{ static_cast<FLOAT>(-w * 0.25f), static_cast<FLOAT>(h * 0.5 - offset) }, ANCH_TYPE::ANCH_P0, m_limit_align_vert);
				m_sample_shape->set_anch_pos(D2D1_POINT_2F{ static_cast<FLOAT>(w * 0.25),  static_cast<FLOAT>(h * 0.5) }, ANCH_TYPE::ANCH_P0 + 1, m_limit_align_vert);
				m_sample_shape->set_anch_pos(D2D1_POINT_2F{ static_cast<FLOAT>(-w * 0.25f), static_cast<FLOAT>(h * 0.5 + offset) }, ANCH_TYPE::ANCH_P0 + 2, m_limit_align_vert);
			}
