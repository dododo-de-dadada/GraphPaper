...
namespace winrt::GraphPaper::implementation
{
	...
	// 図形を表示する.
	void SHAPE_CLOSED::draw_hit(void) noexcept
	{
		...
		hit_draw_square(mid_pt, target, brush);
		...
		hit_draw_square(mid_pt, target, brush);
		hit_draw_square(p[0], target, brush);
		hit_draw_square(p[1], target, brush);
		hit_draw_square(p[3], target, brush);
		hit_draw_square(p[2], target, brush);
	}

	// 図形を表示する.
	void SHAPE_CLOSED::draw(void) noexcept
	{
		ID2D1RenderTarget* const target = SHAPE::m_d2d_target;
		ID2D1SolidColorBrush* const brush = SHAPE::m_d2d_color_brush.get();
	...
	// test_pt	判定される点
	// 戻り値	点を含む判定部位
	uint32_t SHAPE_CLOSED::hit_test(const D2D1_POINT_2F test_pt, const bool/*ctrl_key*/) const noexcept
	{
		...
		if (hit_text_pt(test_pt, p[3], m_hit_width)) {
			return HIT_TYPE::HIT_SW;
		}
		p[1].x = m_start.x + m_lineto.x;
		p[1].y = m_start.y;
		if (hit_text_pt(test_pt, p[1], m_hit_width)) {
			return HIT_TYPE::HIT_NE;
		}
		if (hit_text_pt(test_pt, p[0], m_hit_width)) {
			return HIT_TYPE::HIT_NW;
		}
		...
		if (hit_text_pt(test_pt, bottom, m_hit_width)) {
			return HIT_TYPE::HIT_SOUTH;
		}
		D2D1_POINT_2F right;
		pt_avg(p[1], p[2], right);
		if (hit_text_pt(test_pt, right, m_hit_width)) {
			return HIT_TYPE::HIT_EAST;
		}
		D2D1_POINT_2F left;
		pt_avg(p[0], p[3], left);
		if (hit_text_pt(test_pt, left, m_hit_width)) {
			return HIT_TYPE::HIT_WEST;
		}
		D2D1_POINT_2F top;
		pt_avg(p[0], p[1], top);
		if (hit_text_pt(test_pt, top, m_hit_width)) {
			return HIT_TYPE::HIT_NORTH;
		}
		...
		if (!is_opaque(m_stroke_color) || m_stroke_width < FLT_MIN) {
			...
			const double s_thick = max(m_stroke_width, m_hit_width);
	...
	// 塗りつぶし色を得る.
	// val	得られた値.
	bool SHAPE_CLOSED::get_fill_color(D2D1_COLOR_F& val) const noexcept
	{
		val = m_fill_color;
		return true;
	}
	...
	// 塗りつぶし色に格納する.
	// val	格納する値.
	bool SHAPE_CLOSED::set_fill_color(const D2D1_COLOR_F& val) noexcept
	{
	...
	// hit	部位
	// val	得られた値
	void SHAPE_CLOSED::get_pt_hit(const uint32_t hit, D2D1_POINT_2F& val) const noexcept
	{
		switch (hit) {
	...
	// 境界矩形の左上点を得る.
	void SHAPE_CLOSED::get_bbox_lt(D2D1_POINT_2F& val) const noexcept
	{
	...
	// val	始点
	// 戻り値	つねに true
	bool SHAPE_CLOSED::get_pos_start(D2D1_POINT_2F& val) const noexcept
	{
		val = m_start;
		return true;
	}
	...
	// 戻り値	含まれるなら true
	// 線の太さは考慮されない.
	bool SHAPE_CLOSED::is_inside(const D2D1_POINT_2F lt, const D2D1_POINT_2F rb) const noexcept
	{
	...
	// 位置を移動する.
	// to	移動先へのベクトル
	bool SHAPE_CLOSED::move(const D2D1_POINT_2F to) noexcept
	{
	...
	// hit	部位
	// snap_point
	bool SHAPE_CLOSED::set_pt_hit(const D2D1_POINT_2F val, const uint32_t hit, const float snap_point, const bool /*keep_aspect*/) noexcept
	{
		bool done = false;
		switch (hit) {
		case HIT_TYPE::HIT_SHEET:
		{
			D2D1_POINT_2F pt;
	...
	bool SHAPE_CLOSED::set_pos_start(const D2D1_POINT_2F val) noexcept
	{
	...
	// end_to	対角点への位置ベクトル
	// prop	属性
	SHAPE_CLOSED::SHAPE_CLOSED(const D2D1_POINT_2F start, const D2D1_POINT_2F end_to, const SHAPE* prop) :
		SHAPE_STROKE(prop),
		m_start(start),
		m_lineto(end_to)
	{
	...
	// 図形をデータリーダーから読み込む.
	SHAPE_CLOSED::SHAPE_CLOSED(DataReader const& dt_reader) :
	...
	// 図形をデータライターに書き込む.
	void SHAPE_CLOSED::write(DataWriter const& dt_writer) const
	{
	...
	// val	ある位置の近傍にある頂点
	// 戻り値	見つかったら true
	bool SHAPE_CLOSED::get_pos_nearest(const D2D1_POINT_2F pt, double& dd, D2D1_POINT_2F& val) const noexcept
	{
		bool flag = false;
		D2D1_POINT_2F q[4];
	...
	// p	頂点を格納する配列
	// 戻り値	頂点の個数
	size_t SHAPE_CLOSED::get_verts(D2D1_POINT_2F p[]) const noexcept
	{
		// 左上
		p[0] = m_start;
		// 右上
